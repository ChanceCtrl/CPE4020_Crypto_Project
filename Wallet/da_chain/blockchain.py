...
from blocks import Block, Input, Output, Tx
...
class Blockchain:
    ...
    def force_block(self, check_stop=None):
        ...
        """
        txs = sorted(self.unconfirmed_transactions, key=lambda x: -x[0])[
    ...
    def rollover_block(self, block):
        ...
        for tx in block.txs:
            ...
            for out in tx.outputs:
                self.db.unspent_txs_by_user_hash[str(out.address)].add(
                    (tx.hash, out.hash)
                )
                self.db.unspent_outputs_amount[str(out.address)][out.hash] = int(
                    out.amount
                )
    ...
    def mine_block(self, block, check_stop=None):
        ...
        for n in range(self.max_nonce):
            if check_stop and check_stop():
                ...
                return
            if int(block.hash(nonce=n), 16) <= (
                2 ** (256 - self.db.config["difficulty"])
            ):
